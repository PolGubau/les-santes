...
import {
	MOCK_EVENTS_2026,
	MOCK_FESTIVAL_ID_2026,
} from "../src/entities/event/mock-2026";
...
async function seed() {
	...
	const { error: festivalError } = await supabase.from("festivals").upsert(
		{
			id: FESTIVAL_ID,
			name: process.env.FESTIVAL_NAME ?? "Les Santes 2026",
			city: process.env.FESTIVAL_CITY ?? "Mataró",
			year: Number.parseInt(process.env.FESTIVAL_YEAR ?? "2026", 10),
			starts_on: process.env.FESTIVAL_START ?? "2026-07-17",
